...
- **Screening cooldown & mix** — stocks are stamped with `last_screened_at` after each screen and excluded for a cooldown window (configurable via `screening_cooldown_hours`), ensuring broader universe coverage across cycles. Autonomous re-reviews are also rate-limited per ticker: at least `review_cooldown_days` (default 6) between reviews and at most `max_reviews_per_30_days` (default 5) in a rolling 30-day window. Slack single-ticker reviews bypass this screener gate. Within the eligible pool, `get_screened_universe()` targets a configurable share of fresh (never-reviewed) tickers via `uninvestigated_target_pct` (default ~50%).
