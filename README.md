...
- `GET /api/chat/sessions`, `POST /api/chat/sessions`, `GET /api/chat/sessions/{id}`, `POST /api/chat/sessions/{id}/turns`, `POST /api/chat/sessions/{id}/end` — Chat session CRUD (US-1.9 skeleton)
...
- **Delivered (US-1.6):** Inbound Slack natural language trade commands — e.g. "BUY AAPL", "SELL 10 TSLA", "REVIEW MSFT", "BUY £500 NVDA", "buy apple", "sell google", "review nvidia". Slack Socket Mode listener triggers a full single-ticker pipeline (data → strategy → moderation → risk → execution) with user intent override. **Force buy/sell:** `force buy MSFT`, `!buy AAPL`, `override buy NVDA` bypass risk VETO (logged as OVERRIDDEN). Rejected replies show full pipeline detail (price, strategy reasoning, per-moderator verdicts, risk triggered rules, force hint). Regex-first NL parser (zero cost, supports ticker symbols and single-word company names) with Claude fallback for ambiguous messages. Large order confirmation flow. `SlackCommandLog` audit trail. **Dashboard Commands page** surfaces all command history with stats, filters, and expandable detail rows. CLI: `poetry run python -m src.agents.notifications.slack_trade_listener`. See [Chat & Commands](docs/CHAT_AND_COMMANDS.md).
- **Delivered (US-1.9 skeleton):** Conversational Trading Workflow DB schema (`chat_sessions`, `chat_turns`) + `SessionManager` CRUD stub + dashboard chat API endpoints. No LLM/execution yet — plumbing for future multi-turn conversational trading.
