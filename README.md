...
- **Delivered (US-1.6):** Inbound Slack natural language trade commands — e.g. "BUY AAPL", "SELL 10 TSLA", "REVIEW MSFT", "BUY £500 NVDA". Slack Socket Mode listener triggers a full single-ticker pipeline (data → strategy → moderation → risk → execution) with user intent override; Risk VETO still final. Regex-first NL parser (zero cost) with Claude fallback. Large order confirmation flow. `SlackCommandLog` audit trail. CLI: `poetry run python -m src.agents.notifications.slack_trade_listener`. See [Chat & Commands](docs/CHAT_AND_COMMANDS.md).
