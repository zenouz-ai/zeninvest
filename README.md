...
- **£500 order floor** — BUY and limit-BUY paths are lifted to `min_order_value_gbp` when the requested trade value is below the floor, provided enough cash remains after the cash-floor guard. If there is not enough spendable cash to place the minimum order, the BUY is skipped. REDUCE still requires at least `min_order_value_gbp` unless it is auto-converted to a full SELL. Explicit market SELL and protective stop-loss orders are allowed below the floor so small holdings can still be exited/protected
