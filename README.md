...
- **US-1.9 deployed conversational workflow:** shared conversational trading sessions now span Slack threads and the dashboard `Commands` console, with `chat_actions` + `chat_research_logs`, explicit confirm/reject/expiry flow, portfolio-rule previews, stop updates, cancel proposals, chat SSE events, Slack thread continuity, and a chat-first dashboard operator surface while preserving `SlackCommandLog` as the legacy one-shot audit trail. Session-level spend is now attributable for chat-triggered LLM calls and paid research calls, so each conversation can be measured separately from scheduled-cycle cost.
