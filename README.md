...
- `GET /api/status/` — Next run, cycle_frequency, system state (ACTIVE/CAUTIOUS/HALTED), paused, HALTED auto-recovery progress, and current peak-inflation warning note when active
...
- `GET /api/orders/` — Order history, including off-hours warning notes when orders were submitted outside the regular US market session
...
- `GET /api/system/state`, `POST /api/system/trigger-cycle`, `POST /api/system/pause`, `POST /api/system/resume` — System state and controls, including HALTED recovery streak/target and any active peak-inflation warning note
- `GET /api/commands/`, `GET /api/commands/stats` — Slack trade command audit log (filter by ticker, action, status)
- `GET /api/chat/sessions`, `POST /api/chat/sessions`, `GET /api/chat/sessions/{id}`, `POST /api/chat/sessions/{id}/turns`, `POST /api/chat/sessions/{id}/actions/{action_id}/confirm`, `POST /api/chat/sessions/{id}/actions/{action_id}/reject`, `POST /api/chat/sessions/{id}/end` — Shared Slack/dashboard conversational trading session APIs for `US-1.9`
...
- **US-1.9 deployed conversational workflow:** shared conversational trading sessions now span Slack threads and the dashboard `Commands` console, with `chat_actions`, `chat_research_logs`, and `chat_workflow_steps`; explicit confirm/reject/expiry flow; portfolio-rule previews, stop updates, cancel proposals, and session-level spend attribution; plus a new agentic beta path with planner-led routing, evidence blocks, citations, related-ticker scans, committee views, and a transparent step-by-step workflow rail in `/commands`. Execution remains explicitly confirmed and deterministic even when the chat path uses LLM planning or grounded research. `SlackCommandLog` remains the legacy one-shot audit trail.
...
- **£500 order floor + whole-share preference** — BUY and limit-BUY paths are lifted to `min_order_value_gbp` when the requested trade value is below the floor, provided enough cash remains after the cash-floor guard. Autonomous BUY sizing prefers whole shares with a small configurable overspend tolerance and only falls back to fractional shares when a whole-share order cannot satisfy policy. If there is not enough spendable cash to place the minimum order, the BUY is skipped. REDUCE still requires at least `min_order_value_gbp` unless it is auto-converted to a full SELL. Explicit market SELL and protective stop-loss orders are allowed below the floor so small holdings can still be exited/protected
