...
- **T212 DELETE empty body**: T212 `DELETE /equity/orders/{id}` returns HTTP 200 with empty body. `T212Client._request` handles this by returning `{}`. The retry predicate (`_is_retryable`) only retries 429/5xx/network errors — 4xx client errors fail immediately.
