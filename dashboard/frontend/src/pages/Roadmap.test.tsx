...
describe('timeline section grouping', () => {
  ...
  it('keeps pipeline milestones in their configured horizon buckets', () => {
    ...
    expect(hardening?.columns.Pipeline.map((milestone) => milestone.id)).toContain('US-9.12')
    const mlAdvanced = sections.find((section) => section.topic === 'ML / Advanced')
    expect(mlAdvanced?.columns.Pipeline.map((milestone) => milestone.id)).toContain('US-6.7')
    expect(mlAdvanced?.columns.Pipeline.map((milestone) => milestone.id)).toContain('US-6.2')
  })
  ...
  it('preserves roadmap counts after the reshape', () => {
    ...
    expect(DELIVERED_COUNT).toBe(49)
    expect(PIPELINE_COUNT).toBe(21)
    expect(TOTAL_COUNT).toBe(70)
    expect(PROGRESS_PCT).toBe(70)
