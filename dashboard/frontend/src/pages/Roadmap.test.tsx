...
describe('timeline section grouping', () => {
  ...
  it('keeps pipeline milestones in their configured horizon buckets', () => {
    ...
    expect(hardening?.columns.Delivered.map((milestone) => milestone.id)).toContain('US-9.12')
    const mlAdvanced = sections.find((section) => section.topic === 'ML / Advanced')
    expect(mlAdvanced?.columns.Delivered.map((milestone) => milestone.id)).toContain('US-6.7')
    expect(mlAdvanced?.columns.Delivered.map((milestone) => milestone.id)).toContain('US-6.2')
    expect(mlAdvanced?.columns.Delivered.map((milestone) => milestone.id)).toContain('US-6.4')
    expect(mlAdvanced?.columns.Delivered.map((milestone) => milestone.id)).toContain('US-6.5')
    expect(mlAdvanced?.columns.Future.map((milestone) => milestone.id)).toContain('US-6.3')
  })

  it('preserves roadmap counts after the reshape', () => {
    ...
    expect(DELIVERED_COUNT).toBe(55)
    expect(PIPELINE_COUNT).toBe(16)
    expect(TOTAL_COUNT).toBe(71)
    expect(PROGRESS_PCT).toBe(77)
    expect(MILESTONES.find((milestone) => milestone.id === 'US-9.12')?.wave).toBe(1)
    expect(MILESTONES.find((milestone) => milestone.id === 'US-1.11')?.wave).toBe(3)
