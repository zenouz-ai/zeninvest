...
7. **T212 order status and execution telemetry** — Order status is derived conservatively from T212 broker state: FILLED→filled, PARTIALLY_FILLED→pending while a live remainder still exists, NEW/CONFIRMED/UNCONFIRMED/LOCAL→pending, REJECTED→failed, CANCELLED/CANCELLING→cancelled. Do not assume filled on 200 OK. **Write-before-execute**: live market orders are recorded with status `"submitting"` before the T212 API call and now persist `decision_price`, `filled_quantity`, `remaining_quantity`, and `price=None` until a realised fill is known. `price` should be treated as realised average fill price for live orders, while `slippage_bps` is side-adjusted so positive means worse execution for the operator. **No retry on POST/DELETE**: mutating T212 requests are never automatically retried (T212 has no idempotency keys). Only safe methods (GET) are retried. **`OrderManager.execute_market_order`** issues a **single** `place_market_order` call (no retry loop) so a timeout after the broker accepts cannot double-submit. **Failed execution payloads** (orchestrator trade alerts) still include `quantity`, `price`, and `value_gbp` when the intended size is known, plus the broker HTTP status/body snippet when Trading 212 rejects the request. **Order sync**: at the start of each live cycle and refresh run, `OrderManager.sync_orders_with_t212()` fetches T212 order history and live pending orders, updates fill telemetry, and reconciles stale local rows. If a partial fill's live remainder disappears, the order is marked `cancelled` while preserving filled and remaining quantities. **Conservative fill recovery**: one later approved BUY cycle may retry the exact stored remainder once, but only for autonomous/strategy-driven market BUYs and never for direct/manual orders. **Stale pending market SELL cleanup**: when a later live cycle changes a ticker to `HOLD` or `QUEUED`, the orchestrator cancels any still-live pending market SELL for that ticker so an earlier pre-open exit does not survive the newer decision.
...
10. **Order value floor** — `min_order_value_gbp` is enforced as a minimum BUY ticket size: MARKET BUY and limit-BUY requests below the floor are upgraded to `min_order_value_gbp` when enough cash is available after the cash-floor guard; otherwise the BUY is skipped. SELL, REDUCE, and protective stop-loss SELL orders are allowed below the floor so small positions can be exited or protected. REDUCE is intentionally rare and currently limited to 50% profit trims; if a trim would leave a sub-`small_position_cleanup_value_gbp` residual, execution is converted to a full SELL.
11. **Profit-lock exits** — `sell_min_profit_pct` (default `15%` unrealized gain) is the canonical profit-lock threshold. Once a position crosses it, HOLD is only allowed if a live broker stop protects the full remaining quantity at or above the `+15%` cost-basis line. The orchestrator attempts to place/ratchet that stop in a deterministic pre-strategy pass; if protection cannot be established, it upgrades the position to a full SELL before ordinary strategy/moderation/risk handling. Profit-taking SELL/REDUCE actions above the threshold bypass the ordinary 24h minimum-holding rule. Residual positions below `small_position_cleanup_value_gbp` (default `£200`) are still liquidated immediately in a separate deterministic pre-strategy cleanup pass with no strategy/moderation/risk LLM involvement for that ticker.
...
13. **Intelligent order management** — `StopLossManager` runs after execution each cycle. Stop-loss is placed for BUY when `exec_result.status` in (filled, dry_run) — **not** for `pending` market orders, since T212 requires an existing position and returns 400 if the BUY hasn't filled yet (e.g. placed before market open). Pending BUYs are covered by `place_missing_stops()` in the next cycle once the position exists. **Place missing stops**: `place_missing_stops()` runs before reassessment; positions without a pending stop get one using `default_stop_loss_pct` (or ATR-based when available). Three capabilities:
    ...
    - **Tiered profit-lock floors**: `order_management.profit_lock_tiers` defines gain thresholds and minimum lock percentages (for example `8% -> 5%`). Reassessment and trailing both enforce tier floors by applying the tighter stop.
    ...
    - All adjustments logged to `stop_loss_adjustments` table and emitted as `order_adjustment` Slack notifications. Tiered adjustments additionally persist `tier_gain_trigger_pct`, `tier_min_lock_pct`, and `tier_rule_label` for audit.
...
Intraday mode also runs a dedicated refresh lane: Mon–Fri `09:50`, `10:10`, `12:20`, `12:40`, `15:05`, `15:25` America/New_York, plus Sat/Sun `17:00` America/New_York. These refresh runs sync broker order truth, write a fresh `portfolio_snapshots` row, warm held/pending/queued market data, and run deterministic stop/profit-lock maintenance without screening new names. Other scheduled jobs remain: daily snapshot 21:30 UTC, weekly report Fri 22:00 UTC, instrument refresh Sun 12:00 UTC, strategy episode scan daily 02:00 UTC (auto-publishes confirmed attribution episodes from recent git history); on scheduler startup, if no episodes exist, a one-time 30-day backfill runs automatically.
...
- **Order management**: `enabled`, `default_stop_loss_pct: -10`, `reassess_stops`, `trailing_stops` (enabled, trail_pct), `profit_lock_tiers` (enabled, global tier floors), `limit_orders` (enabled, offset_pct, validity), ATR multiplier, min/max stop distance, only_tighten_stops
...
- **US-1.9** Conversational Trading Workflow MVP implementation landed in repo — shared Slack/dashboard conversational session backend, `chat_actions` + `chat_research_logs`, explicit confirm/reject/expiry action state machine, bounded natural-language intents (research, strategy/direct trade, cancel, stop update, portfolio-rule batch preview), dashboard `Commands` page repurposed into a chat-first operator console, and legacy `SlackCommandLog` preserved as separate one-shot audit history. Final validation/deployment checklist still pending before the story is treated as fully closed.
- **US-1.10** Evolution Planner (partially delivered, Phase 1 planner-only) — separate evolution workflow domain (`evolution_requests`, `evolution_messages`, `evolution_plans`, `evolution_runs`, `evolution_artifacts`, `evolution_approvals`, `evolution_deployments`), authenticated dashboard Evolution page, deterministic intent normalization, static repo-context mapping, risk classification, validation matrix, clarification loop, and planner-only policy gates on build/deploy approval attempts. Branch execution, code changes, build workers, and deploy authority remain future phases.
...
- **US-3.5a** Tiered Profit-Lock Floors — global `order_management.profit_lock_tiers`, integrated ATR/trailing stop precedence (`max(atr_stop, tier_floor, profit_lock_floor)`), audited tier metadata in stop-loss adjustments, and dashboard schema exposure for adjustment history
