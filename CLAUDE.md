...
└── frontend/              # React + Vite + Tailwind frontend
    └── src/
        ├── pages/         # 10 pages: Dashboard, Universe, Portfolio, Opportunity, OrderManagement, RunHistory, WorldNews, Commands, Costs, Roadmap
...
10. **Order value floor** — `min_order_value_gbp` applies to BUY/REDUCE/limit paths; for MARKET BUYs, the floor check uses the *target trade value* (pre share flooring) so rounding does not create tiny sub-£500 logged values. Explicit market SELL decisions may execute below the floor so small positions can be fully exited. Protective stop-loss SELL orders are also allowed below the floor so small positions remain risk-protected. REDUCE that would leave a sub-£500 residual is auto-converted to full SELL.
11. **Stop-loss** — automatically placed after every BUY using Claude's `stop_loss_pct` (GTC validity). `place_stop_loss()` accepts optional `current_price_gbp` for GBP-denominated `value_gbp` logging when native price is USD. **Before SELL/REDUCE:** `OrderManager.cancel_conflicting_stops(ticker)` cancels any pending stop-loss orders for the ticker before placing the market order (T212 reserves shares for pending stops, blocking concurrent sells). **Idempotent cancel:** HTTP 404 and common 400/409 “already gone / not pending” responses are treated as success so the SELL can proceed. If cancellation fails with a non-idempotent error, the SELL is aborted. **SELL/REDUCE quantity** is clamped to `T212Client.get_position(ticker)` after stops are cleared to avoid oversize sells. For `liquidate_all()`, stop cancellation is fail-open. After REDUCE, `place_missing_stops()` re-places a stop for remaining shares in the same cycle.
...
- **slack_listener.py** — Socket Mode handler for inbound trade commands; resolves bot's own `user_id` via `auth.test` on startup and filters out self-messages (prevents cascading reply loops); processes BUY/SELL/REVIEW commands in background threads; large-order confirmation flow with expiry; console completion log after every command; graceful shutdown via `threading.Event`.
...
- **trade_command_parser.py** — Regex-first NL parser with Claude fallback; extracts action (BUY/SELL/REVIEW), ticker, optional quantity/amount_gbp, and `force` flag from free-text Slack messages. Force prefixes: `force buy`, `override buy`, `!buy`.
...
- **Dashboard**: `enabled`, `events_enabled`, `cors_origins` (list of allowed CORS origins; defaults to localhost when absent); `DASHBOARD_API_KEY` env var — when set, all `/api/*` endpoints require matching `X-API-Key` header (US-7.1); unset = unauthenticated dev mode with startup warning. Frontend reads `VITE_API_KEY` at build time (Docker build arg) or falls back to `localStorage.getItem('dashboard_api_key')`; **nav “API key” modal** saves/clears that key and reloads. **Axios response interceptor** + **SSE 403** set a global **auth banner** (Retry bumps SSE reconnect nonce; Dismiss clears flag). **`APIKeyMiddleware`** validates keys with **`hmac.compare_digest`** (UTF-8, same length only). **`GET /api/events/stream` uses `fetch()` + stream parsing (not `EventSource`)** so the same `X-API-Key` can be sent; exponential backoff on disconnect (403 does not reconnect). `public_routes` (list of `/api/*` path prefixes) — GET requests to these prefixes bypass auth for demo/public exposure; write endpoints (`/api/system/*`, `/api/runs/trigger*`) are always protected regardless of this list. `/api/orders/health` unresolved failures: cleared only by a later **filled** or **cancelled** order for the same `(ticker, action, order_type)` — not by `dry_run`.
...
- **US-1.7** Dashboard full spec (full API + 10 pages)
...
- **US-1.6** Slack NL Trade Commands — inbound BUY/SELL/REVIEW via Slack Socket Mode; single-ticker pipeline (data→strategy→moderation→risk→execution) with user-intent override; **force buy/sell** (`force buy`, `!buy`, `override buy`) bypasses explicit moderation/risk blocks for Slack commands (logged as OVERRIDDEN); regex-first NL parser with Claude fallback; `SlackCommandLog` audit table; `CommandGateway` evolved from scaffold; `resolve_ticker_to_t212()` utility; large order confirmation flow; FX-aware GBP sizing for explicit `£` orders on `_US_EQ` / OTC names; enhanced rejected replies (full pipeline detail, price, per-moderator verdicts, risk rules, force hint); graceful shutdown via `threading.Event`; console completion log after every command; reply formatter; CLI entry `poetry run python -m src.agents.notifications.slack_trade_listener`; Docker deploy now includes an always-on `slack-listener` service; 64 new tests
