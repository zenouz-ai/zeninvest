...
7a. **T212 empty-body responses** — T212 DELETE `/equity/orders/{id}` returns HTTP 200 with an empty body on success. `_request` handles this by returning `{}` when the response text is empty/whitespace instead of calling `.json()`. The retry predicate (`_is_retryable`) only retries transient errors (429, 5xx, network); 4xx client errors (including 404 "Order not found") fail immediately. As defense-in-depth, `cancel_conflicting_stops` unwraps tenacity `RetryError` to check the underlying exception for 404 before treating the cancellation as a hard failure.
7b. **Dashboard order health + stale pending reconciliation** — `/api/orders/health` reports unresolved failed orders and pending health metrics (`pending_local_count`, `pending_live_count`, `stale_pending_count`). Unresolved failures are not raw historical failures: they are recent failures (default 7 days) or failures without a later successful order for the same `ticker+action+order_type`. On health checks, stale local pending stop rows missing from live T212 pending orders are reconciled to `cancelled`.
