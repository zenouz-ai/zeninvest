...
Intraday mode also runs a dedicated refresh lane: Mon–Fri `09:50`, `10:10`, `12:20`, `12:40`, `15:05`, `15:25` America/New_York, plus Sat/Sun `17:00` America/New_York. These refresh runs sync broker order truth, write a fresh `portfolio_snapshots` row, warm held/pending/queued market data, and run deterministic stop/profit-lock maintenance without screening new names. Other scheduled jobs remain: daily snapshot 21:30 UTC, weekly report Fri 22:00 UTC, instrument refresh Sun 12:00 UTC.
...
- **US-7.7** Dashboard HTTPS Domain & Canonical Access — canonical `https://zeninvest.zenouz.ai` behind Cloudflare + nginx, internal-only dashboard app, no public raw `:8000`, narrow anonymous read-only surface, proxy-safe operator auth
