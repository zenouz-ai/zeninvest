...
| 2026-03-10 | Added fresh vs investigated screening mix | `get_screened_universe()` now targets a configurable share of never-investigated tickers (`uninvestigated_target_pct`, default 0.5) while preserving sector and cap-tier constraints, so each cycle explores new names instead of only revisiting prior decisions. |
