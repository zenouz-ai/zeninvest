...
- ~~Dashboard Phase 1~~ → FastAPI backend (runs, universe, portfolio, orders, SSE events), React frontend now grown into a 12-page authenticated operator surface (including Commands, World News, Insights, Roadmap, and Evolution Planner); stabilisation complete; US-1.8 delivered (Docker service, port 8000; SPA served by FastAPI; activity feed works from VPS IP)
