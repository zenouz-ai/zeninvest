...
**Codebase:** ~29,500 lines Python across 29 test files (311 tests)
...
|---|---------|------|----------------|
| C1 | **Dashboard has no authentication** | Anyone with network access can trigger live trades via `POST /api/runs/trigger-live` | Add API key or token-based auth before exposing beyond localhost |
| C2 | **No US holiday calendar** | Orders may be submitted on market holidays, likely rejected by T212 but wastes API budget | Integrate a holiday calendar check in scheduler |

### 13.2 Important

| # | Finding | Risk | Recommendation |
|---|---------|------|----------------|
| I1 | **Partial fills not resubmitted** | Intended position size not fully achieved | Consider resubmission logic for unfilled remainder in next cycle |
| I2 | **No slippage/market impact modelling** | Market orders execute at best available; no VWAP/TWAP | Acceptable for practice; review before live deployment |
| I3 | **Position sizing is pure LLM output** | Claude decides allocation % with no quantitative framework | Consider Kelly fraction, risk-parity, or vol-targeting overlay |
| I4 | **Orchestrator has no integration tests** | Full pipeline regressions may go undetected | Add end-to-end `run_cycle()` test with mocked APIs |
| I5 | **Opportunity optimizer under-tested** | Queue promotion, TTL expiry, swap logic have minimal coverage | Expand to cover edge cases (full capacity, expired queues) |

### 13.3 Minor / Cosmetic

| # | Finding | Notes |
|---|---------|-------|
| M1 | Stop-loss placed optimistically for pending BUYs | By design; may result in orphaned stops if BUY rejected by T212 |
| M2 | Dry-run dedup doesn't prevent live duplicate | Mitigated by scheduling — unlikely to run both within 5 min |
| M3 | `_get_previous_ewma` makes N+1 queries | One query per ticker; could be batch query for performance |
| M4 | Queued tickers may be analyzed twice in same cycle | Once as candidate, once as queued re-evaluation (Phase 3) |
...
*Report generated by comprehensive codebase audit, 2026-03-17.*
