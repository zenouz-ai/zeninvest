...
|-------|------------------|--------|---------|
| **Production Access & Safety** | delivered `US-7.8`, delivered `US-7.7`, delivered `US-7.5`, delivered `US-7.3`, delivered `US-7.2` | **Delivered through the safe public demo + execution-quality slice** | Canonical ingress, public-surface hardening, execution telemetry, and conservative fill recovery are all shipped |
...
| **US-1.7** | Dashboard & Visualisation | Web dashboard base: 10 core pages (Home with state badge, Universe, Run History, Portfolio, Opportunity, Order Mgmt, Commands, World News, Costs, Roadmap) plus the full operator API (decisions, moderation, risk, opportunity, outcomes, stop-loss, performance, costs, api-usage, system, refresh status/trigger, order health); later extended by `US-1.10` with the dedicated Evolution Planner workspace. The shipped surface now includes refresh-aware dashboard state, order reconciliation visibility, and the dedicated intraday refresh lane. | Full operational visibility; personal quant experience | **Delivered** |
...
| **US-1.8** | Dashboard VPS Deployment | Original Dockerized VPS deployment path for the dashboard; the raw VPS/IP route is now superseded in production by US-7.7 canonical HTTPS ingress and the safer public demo posture from US-7.8. See `docs/DASHBOARD_DEPLOYMENT.md`. | Operational visibility on live VPS | **Delivered** |
...
| **US-7.8** | Safe Public Demo Dashboard | Public visitors now see the full tab map, but every anonymous page is either a sanitized live projection or a disabled preview surface with dedicated redaction, ingress hardening, and CI checks. | Lets the product be demonstrated safely without exposing operator controls, raw reasoning, or sensitive account data | **Delivered** |
...
| 7.8 | Safe public demo dashboard | High | Easy–Med | M | US-7.7 + US-7.1 | **P1** |
...
- [x] Repo-context retrieval grounded in roadmap, architecture, governance, deployment, and likely code areas via deterministic static mappings
...
**US-7.8: Safe Public Demo Dashboard**
**Value:** Public visitors can understand the product without gaining access to operator controls or sensitive portfolio data
**Effort:** Medium (1-2 days)
**Data Sources:** Same DB as agent (shared volume), but only through dedicated sanitized read models
**Stage:** Delivered (2026-03-29)

**Status (2026-03-29):** Delivered. The anonymous dashboard now renders the full product navigation, but every tab is intentionally constrained to either sanitized live data or a disabled preview surface. Dedicated public serializers back Universe, Portfolio, Runs, Opportunity, Costs, and World News; Order Management, Chat, and Evolution expose non-functional preview shells only; private actions, exact balances, exact quantities, broker IDs, raw reasoning, and operator-only controls remain authenticated. Nginx now adds security headers and rate limits for login and `/api/public/*`, and CI verifies the frontend build, backend/public tests, `bandit`, `pip-audit`, and `npm audit`.

**Acceptance Criteria:**
- [x] Signed-out visitors see all product tab names without gaining access to private operator routes
- [x] Public live pages use dedicated read-only serializers with server-side caps and fail-soft redaction
- [x] Order Management, Chat, and Evolution expose preview-only surfaces with all live functionality disabled
- [x] Public pages hide exact account values, exact quantities, action columns, broker identifiers, and raw reasoning
- [x] Nginx enforces security headers plus rate limits for login and public endpoints
- [x] Frontend/backend tests and CI checks cover the public demo surface and security posture
