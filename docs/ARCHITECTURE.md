...
React frontend (SPA, served by FastAPI when dist/ exists)
    |
    +-- 12 pages: Dashboard Home (skeleton loading, alert banner, metric cards, positions with sparklines, activity feed, hardening warnings), Universe (deep-linkable /universe/:ticker; public mode shows capped sanitized rows), Run History (public mode shows capped sanitized runs), Portfolio (sparklines, public normalized mode + operator-only Force Sell), Opportunity Pipeline (public mode shows capped sanitized examples), Insights (public market-guidance view plus private strategy-attribution review), Order Management (including off-hours warning notes; public page is preview-only), Research (`/commands`: chat-first conversational operator console with shared Slack/dashboard sessions, planner-led agentic beta routing, live agent-activity rail, evidence panels, degraded-turn warning cards, pending proposal rail, research trace, session spend, and a secondary Legacy Slack Audit tab that is not the full conversation archive; public page is preview-only), World News (public read-only macro archive), Costs (public aggregate totals), Roadmap & Architecture, Evolution (public page is preview-only)
    +-- Public nav: full product tab map; each page is either sanitized live data or a preview-only surface, while operator nav adds Dashboard as the authenticated home
...
**Authentication (US-7.1 + US-7.7 + US-7.8):** The dashboard uses a public/private split. Anonymous read-only routes live under `/api/public/*`, and the anonymous page surface is deliberately constrained to sanitized projections or disabled preview surfaces. Public live routes now cover overview docs/performance, universe, portfolio, runs, opportunity, sanitized insights guidance, costs, and world-news/macro data; Order Management, Chat, Evolution, and private strategy-attribution review expose preview shells only. All other `/api/*` routes require operator login and a signed `HttpOnly` session cookie. Operator actions such as cycle triggers, pause/resume, order management, chat execution, evolution planning, strategy-attribution review, and Force Sell remain private even when their corresponding read-only pages are public. Operator auth is configured with `DASHBOARD_OPERATOR_USERNAME`, `DASHBOARD_OPERATOR_PASSWORD_HASH`, `DASHBOARD_SESSION_SECRET`, and optional localhost-only `DASHBOARD_INSECURE_DEV_MODE=true`. Operator login is blocked over plain HTTP outside localhost dev mode. Production ingress is HTTPS behind Cloudflare + Nginx on `zeninvest.zenouz.ai`, with Nginx enforcing canonical host/scheme access, forwarding `X-Forwarded-Proto: https` to the internal-only dashboard app, adding security headers, and rate-limiting login plus `/api/public/*`. The frontend no longer injects shared secrets at build time or stores dashboard credentials in `localStorage`. Axios and SSE treat `401/403` from protected APIs as a signed-out state. SSE uses `fetch()` + stream parsing with credentials included.
...
- Tiered profit-lock floors (`order_management.profit_lock_tiers`) ratchet minimum protected profit as gains increase. Stop maintenance applies the tighter floor across ATR reassessment and trailing logic.
