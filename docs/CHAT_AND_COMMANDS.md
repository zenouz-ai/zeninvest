...
- [x] **US-1.9 Commands console upgrade**: `/commands` is now chat-first for authenticated operators, with shared Slack/dashboard conversation sessions, live thread + composer, pending proposal controls, research trace, and a secondary legacy command-history tab so conversational workflow evidence stays separate from one-shot `SlackCommandLog` audit rows.
