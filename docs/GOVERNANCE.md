...
- **`OrderManager.execute_market_order`** issues **one** `POST /equity/orders/market` per decision (no application-level retry loop on top of the client). A network timeout after the broker has accepted an order remains an edge case for manual reconciliation via T212 / `sync_order_status_from_t212`, not a second POST.
