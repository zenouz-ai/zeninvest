...
- **`OrderManager.execute_market_order`** issues **one** `POST /equity/orders/market` per decision (no application-level retry loop on top of the client). A network timeout after the broker has accepted an order remains an edge case for manual reconciliation via T212 / `sync_orders_with_t212()`, not a second POST.
- **Write-before-execute**: Market orders are recorded in the DB with status `"submitting"` *before* the T212 API call. If the process crashes after T212 accepts but before the response is processed, the orphaned record enables crash recovery. The record is updated to the actual T212 status after the API returns. **Trade execution notifications** include intended `quantity` / `value_gbp` on failure when known (orchestrator fallback uses `calculate_quantity` if the execution dict omits quantity).
...
- **Order status from T212**: The T212 API response `status` is mapped conservatively: FILLED→filled, PARTIALLY_FILLED→pending while a live remainder still exists, NEW/CONFIRMED/etc→pending, REJECTED→failed, CANCELLED/CANCELLING→cancelled. Do not assume a 200 OK implies execution. Market-order rows now persist `decision_price`, `filled_quantity`, `remaining_quantity`, and side-adjusted `slippage_bps`, with `price` reserved for the realised average fill price. **Order sync**: at the start of each live cycle and refresh run, `OrderManager.sync_orders_with_t212()` refreshes broker truth, updates fill telemetry, and reconciles stale pending rows. If a partial fill's live remainder disappears, the order is marked `cancelled` while preserving the filled and remaining quantities for audit. **Conservative fill recovery**: when enabled, one later approved BUY cycle may resubmit the exact stored remainder once, but only for autonomous/strategy-driven market BUYs and only if the newly desired size fully covers the remainder. **Broker rejection detail**: failed market and stop orders preserve the T212 HTTP status/body snippet in the stored error/notification payload so operators can distinguish minimum-size, reserved-share, and other 400-series rejects. **Non-tradable BUY quarantine**: if T212 rejects a BUY with a 400 `instrument-invisible` / `Instrument can not be traded` payload, the instrument is marked `data_available=False` so future screening and execution skip it.
...
- **FX-aware BUY quantity and value logging**: For `_US_EQ` instruments, BUY quantity is calculated using a GBP-equivalent price (`current_price × GBP/USD scale`) derived from `_compute_position_value_scale()` in the orchestrator. This ensures sizing is based on GBP rather than raw USD. Autonomous BUYs prefer whole-share quantities when feasible, with fractional shares used only as fallback. `Order.value_gbp` for BUY remains the GBP target amount. Stop-loss `Order.value_gbp` uses `current_price_gbp` when available for accurate GBP logging. Config: `trading.fx_aware_quantity: true` (default on; falls back to scale=1.0 on empty portfolio).
...
- The REDUCE action is supported alongside BUY and SELL, but it is intentionally rare. **Min holding period**: Risk normally blocks REDUCE/SELL on positions held &lt; 24h unless over max_single_stock or max_sector, but profit-taking SELL/REDUCE actions above the profit-lock threshold bypass that rule. **Profit-lock SELL policy**: `sell_min_profit_pct` (default `15%`) is the canonical threshold. Once a position crosses it, HOLD is only allowed if a live broker stop protects the full remaining quantity at or above the profit-lock line; otherwise the orchestrator deterministically exits the position. **Small-position cleanup**: holdings below `small_position_cleanup_value_gbp` (default `£200`) are liquidated immediately in a pre-strategy deterministic pass. The ticker is removed from current-position analysis, no strategy/moderation/risk LLM stage runs for it, and the broker-reported live share quantity is sent straight to Trading 212 so the system runs a single cleanup procedure without leaving residual dust from allocation rounding. **Min order value**: MARKET BUY and limit-BUY requests below `min_order_value_gbp` are upgraded to the floor when enough cash remains after the cash-floor guard; otherwise the BUY is skipped. SELL, REDUCE, and protective stop-loss SELL orders are allowed below the floor to fully exit/protect small holdings. **Residual cleanup safeguard**: if a REDUCE would leave a position below `small_position_cleanup_value_gbp`, execution is converted to a full SELL. **Reduction tiers**: REDUCE is rounded to the configured tier and any other trim request is downgraded to HOLD; current policy is 50%-only. **Profit-trim thresholds**: REDUCE 50% requires at least `reduce_50_min_profit_pct` unrealized gain (default `40%`). **REDUCE trigger policy**: REDUCE is downgraded to HOLD unless `exit_trigger_type="profit_trim"`.
...
- **Tiered profit-lock floors**: `order_management.profit_lock_tiers` defines global gain thresholds and minimum lock percentages (for example `8% -> 5%`). Reassessment and trailing both enforce tier floors by taking the tighter protective stop. Tier metadata is persisted in `stop_loss_adjustments` (`tier_gain_trigger_pct`, `tier_min_lock_pct`, `tier_rule_label`) for audit.
- **Software trailing stops**: `StopLossManager.apply_trailing_stops()` tracks a high-water mark (HWM) per position. When price exceeds previous HWM, the stop ratchets up to `HWM × (1 - trail_pct/100)`. Implemented as cancel-then-place: the old T212 stop is cancelled first (T212 allows only one pending stop per instrument), then the new stop is placed. If the new placement fails, an emergency stop at the old price is immediately re-placed to restore protection. Ratchet is skipped when computed stop ≥ current price (`trailing_ratchet_invalid` recorded in `stop_loss_adjustments`).
...
### 6.2a Force Buy/Sell via Slack (Risk Override)

In Slack, prefix any trade command with `force`, `override`, or `!` to bypass the risk VETO:

```
force buy MSFT          # Bypasses risk VETO for BUY
force buy 2 shares of AAPL
!buy £500 NVDA
override sell TSLA
```
...
| `research_logs` | Agentic research tool calls (US-4.4) | `cycle_id`, `member` (strategy/skeptic/risk), `ticker`, `tool_name`, `query`, `provider`, `cache_hit`, `latency_ms`, `cost_usd` ($0.005/paid call); per-member caps 20/8/7, total 35/cycle; budget monitoring via ResearchBudget. Research costs surfaced on dashboard Costs page as separate "Agentic Research" band and in `/api/research/summary` by member/tool/provider. |
